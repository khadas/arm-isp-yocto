...
static int media_enum_links(struct media_device *media)
{
    ...
    for (id = 1; id <= media->entities_count; id++) {
        ...
        links.pads = (struct media_pad_desc *)calloc(entity->info.pads, sizeof(struct media_pad_desc));
        links.links = (struct media_link_desc *)calloc(entity->info.links, sizeof(struct media_link_desc));
...
static void media_debug_default(const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    vprintf(fmt, args);
...
static struct media_device *__media_device_new(void)
{
    ...
    media = (struct media_device *)calloc(1, sizeof(*media));
    if (media == NULL)
        return NULL;
                ...
                const char *devnode)
{
    ...
    size = (media->entities_count + 1) * sizeof(*media->entities);
    entity = (struct media_entity *)realloc(media->entities, size);
    if (entity == NULL)
        return -ENOMEM;
