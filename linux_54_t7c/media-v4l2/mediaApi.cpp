...
int mediaStreamInit(media_stream_t *stream, struct media_device * dev)
{
    ...
    printf("media devnode: %s", stream->media_dev->devnode);

    if (NULL == stream->media_dev) {
        printf("new media dev fail");
        return -1;
    }
    ...
    if (0 != media_device_enumerate(stream->media_dev) ) {
        printf("media_device_enumerate fail");
        return -1;
    }
    ...
    for (int i = 0, j = 0; i < node_num; ++i) {
        ...
        printf("ent %d, name %s ", i, ent->info.name);
        if (strstr(ent->info.name, "csiphy")) {
            ...
            if (j == 0)
                ...
                printf("invalid index %d, ent %s", j, ent->info.name);
            j++;
        }
    }
    ...
    if (NULL == stream->video_param) {
        printf("get video_param fail");
    }

    int ret = v4l2_video_open(stream->video_ent0);
    printf("%s open video0 fd %d ", __FUNCTION__, stream->video_ent0->fd);

    if (stream->video_ent1) {
        ret = v4l2_video_open(stream->video_ent1);
        printf("%s open video1 fd %d ", __FUNCTION__, stream->video_ent1->fd);
    }
    if (stream->video_ent2) {
        ret = v4l2_video_open(stream->video_ent2);
        printf("%s open video2 fd %d ", __FUNCTION__, stream->video_ent2->fd);
    }
    if (stream->video_ent3) {
        ret = v4l2_video_open(stream->video_ent3);
        printf("%s open video3 fd %d ", __FUNCTION__, stream->video_ent3->fd);
    }

    printf("media stream init success");
    return 0;
}
...
int createLinks(media_stream_t *stream)
{
    ...
    if (!sink_pad) {
        printf("Failed to get isp sink pad[0]");
        return rtn;
    }

    src_pad = (struct media_pad*)media_entity_get_pad(stream->adap_ent, src_pad_idx);
    if (!src_pad) {
        printf("Failed to get adap_ent src pad[1]");
        return rtn;
    }
    ...
    if (!sink_pad) {
        printf("Failed to get adap sink pad[0]");
        return rtn;
    }

    src_pad = (struct media_pad*)media_entity_get_pad(stream->csiphy_ent, src_pad_idx);
    if (!src_pad) {
        printf("Failed to get csiph src pad[1]");
        return rtn;
    }

    rtn = media_setup_link( stream->media_dev, src_pad, sink_pad, flag);
    if (0 != rtn) {
        printf( "Failed to link adap with csiphy");
        return rtn;
    }
    ...
    if (!src_pad) {
        printf("Failed to get sensor src pad[0]");
        return rtn;
    }

    sink_pad = (struct media_pad*)media_entity_get_pad(stream->csiphy_ent, sink_pad_idx);
    if (!sink_pad) {
        printf("Failed to get csiph sink pad[1]");
        return rtn;
    }

    rtn = media_setup_link( stream->media_dev, src_pad, sink_pad, flag);
    if (0 != rtn) {
        printf( "Failed to link sensor with csiphy");
        return rtn;
    }
...
int setSdFormat(media_stream_t *stream, stream_configuration_t *cfg)
{
    ...
    rtn = v4l2_subdev_set_format(stream->sensor_ent,
          &mbus_format, 0, which);
    if (rtn < 0) {
        printf("Failed to set sensor format");
        return rtn;
    }
    ...
    rtn = v4l2_subdev_set_format(stream->csiphy_ent,
          &mbus_format, 0, which);
    if (rtn < 0) {
        printf("Failed to set csiphy pad[0] format");
        return rtn;
    }
    ...
    rtn = v4l2_subdev_set_format(stream->csiphy_ent,
          &mbus_format, 1, which);
    if (rtn < 0) {
        printf("Failed to set csiphy pad[1] format");
        return rtn;
    }
    ...
    if (rtn < 0) {
        printf("Failed to set adap pad[0] format");
        return rtn;
    }
    ...
    rtn = v4l2_subdev_set_format(stream->adap_ent,
          &mbus_format, 1, which);
    if (rtn < 0) {
        printf("Failed to set adap pad[1] format");
        return rtn;
    }
    ...
    if (rtn < 0) {
        printf("Failed to set isp pad[0] format");
        return rtn;
    }
#if 0
    rtn = v4l2_subdev_set_format(stream->isp_ent,
          &mbus_format, 1, which);
    if (rtn < 0) {
        printf("Failed to set isp pad[1] format");
        return rtn;
    }
#endif
    printf("%s success -- . \n", __FUNCTION__);
...
int setImgFormat(media_stream_t *stream, stream_configuration_t *cfg)
{
    ...
    for (int i = 0; i < 4; ++i) {
        if (cfg->vformat[i].width > 0 && cfg->vformat[i].height > 0) {
            ...
            if (rtn < 0) {
                printf("Failed to set video fmt, ret %d. \n", rtn);
                return rtn;
            }
        }
    }
...
int setDataFormat(media_stream_t *camera, stream_configuration_t *cfg)
{
    ...
    if (rtn < 0) {
        printf("Failed to set video fmt, ret %d", rtn);
        return rtn;
    }
...
int media_set_wdrMode(media_stream_t *camera, uint32_t wdr_mode)
{
    ...
    if (wdr_mode != ISP_SDR_DCAM_MODE) {
        ...
        if (rtn < 0) {
            printf("Failed to set sensor wdr mode");
            return rtn;
        }
    }
    ...
    if (rtn < 0) {
        printf("Failed to set adapter wdr mode");
        return rtn;
    }
    ...
    if (rtn < 0) {
        printf("Failed to set isp wdr mode");
        return rtn;
    }
...
int mediaStreamConfig(media_stream_t * stream, stream_configuration_t *cfg)
{
    ...
    printf("%s %dx%d ++", __FUNCTION__, cfg->format.width, cfg->format.height);

    rtn = setSdFormat(stream, cfg);
    if (rtn < 0) {
        printf("Failed to set subdev format");
        return rtn;
    }
    ...
    printf("Success to config media stream ");
    return 0;
}
