...
namespace act {
    ...
    CATLError CCommandManager::Initialize(const flags32&) {
        ...
        if ( isXMLLoaded() ) {
            //SysLog()->LogDebug(LOG_CONTEXT, "file AmlControl.xml has been loaded");
        } else {
            //SysLog()->LogInfo(LOG_CONTEXT, "file AmlControl.xml must be loaded via front-end");
        }
        return result;
    }
    ...
    CATLError CCommandManager::CommandLutRead( const TSmartPtr< CRequestParser > & clientRequest, TSmartPtr< CResponseWriter > answer ) {
        ...
        SysLog()->LogDebug(LOG_CONTEXT, "calibration read {page: %d, section: %d type: 0, size: " FSIZE_T "}", page, section, sz);
    ...
    CATLError CCommandManager::CommandLutWrite( const TSmartPtr< CRequestParser > & clientRequest, TSmartPtr< CResponseWriter > ) {
        ...
        SysLog()->LogDebug(LOG_CONTEXT, "calibration write {page: %d, section: %d type: 0, size: " FSIZE_T "}", page, section, sz);
