        ...
        for (DWORD i = 0; i < numDevs; i++) {
            ...
            if (ftStatus == AML_OK) {
                if (name == string("FTDI,")+Description+","+SerialNumber) {
                    AML_STATUS res = FT_Open(i,&hFTDI);
                    if (res!=AML_OK) {
                        hFTDI = 0;
                    ...
                    if (AML_OK != FT_SetLatencyTimer(hFTDI,1) ||
                        AML_OK != FT_SetUSBParameters(hFTDI,65536,65536) ||
                        AML_OK != FT_SetTimeouts(hFTDI,5000,1000)) {
...
void FTDI::reset()
{
    ...
    if (hFTDI) {
        AML_STATUS res = FT_ResetDevice(hFTDI);
        if (AML_OK != res) {
...
void FTDI::set_bitmode(UInt8 mask, UInt8 mode)
{
    ...
    if (hFTDI) {
        AML_STATUS res = FT_SetBitMode(hFTDI, mask, mode);
        if (AML_OK != res) {
...
void FTDI::set_baudrate(UInt32 baudrate)
{
    ...
    if (hFTDI) {
        AML_STATUS res = FT_SetBaudRate(hFTDI, baudrate);
        if (AML_OK != res) {
...
void FTDI::write(const std::vector<UInt8>& data)
{
    ...
    if (hFTDI) {
        ...
        do {
            ...
            total += written;
        } while (res == AML_OK && data.size() != total);
        if (res != AML_OK) {
...
const DevList FTDI::get_devices()
{
    ...
    if (ftStatus == AML_OK) {
        ...
        for (DWORD i = 0; i < numDevs; i++) {
            ftStatus = FT_GetDeviceInfoDetail(i, &Flags, &Type, &ID, &LocId, SerialNumber, Description, &ftHandleTemp);
            if (ftStatus == AML_OK && strlen(Description)) {
...
const DevList FTDI::get_devices()
{
    ...
    struct aml_context* hFTDI = ftdi_new();
    if (!hFTDI) {
