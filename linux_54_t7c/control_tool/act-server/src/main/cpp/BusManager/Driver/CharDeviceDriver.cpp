    ...
    void CCharDeviceDriver::rx_thread_processing() {
        ...
        while(!rx_thread_stop) {
            ...
            for (list_type::iterator it = transfer_list.begin(); it != transfer_list.end();) {
                ...
                if (armcpp11::DurationCast<armcpp11::MilliSeconds>(now - transaction.sent_time).Count() > transaction.packet->timeout_ms &&
					armcpp11::DurationCast<armcpp11::MilliSeconds>(now - transaction.sent_time).Count() < transaction.packet->timeout_ms * 60) { // timeout for packet
                    TSmartPtr<CTransferPacket> packet = it->second.packet;    // get packet from the list
