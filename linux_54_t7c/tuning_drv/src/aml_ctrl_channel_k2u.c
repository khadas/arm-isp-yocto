...
#define LOG( ... )
#define CTRL_CHANNEL_BIDIRECTION
...
void ctrl_channel_handle_command( uint8_t type, uint8_t command, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
    ...
    LOG( LOG_INFO, "kernel api_command: cmd_type: %u, cmd_id: %u, cmd_direction: %u, cmd_value: %u.\n", p_cmd->cmd_type, p_cmd->cmd_id, p_cmd->cmd_direction, p_cmd->cmd_value );
	...
	if ( direction == COMMAND_GET) {
		int rc = 0, icount = 0;
	    while ( rc == 0) {
			rc = ctrl_channel_read((char *)ret_value, 4);
			mdelay(5);
		    if ( icount ++ > 100)
				break;
	    }
...
void ctrl_channel_handle_api_calibration( uint8_t type, uint8_t id, uint8_t direction, void *data, uint32_t data_size, uint32_t *ret_value )
{
    ...
    if ( direction == COMMAND_GET) {
        int rc = 0, icount = 0;
        while ( rc == 0) {
            rc = ctrl_channel_read((char *)data, data_size);
            mdelay(5);
            if ( icount ++ > 100)
            break;
        }
...
uint8_t application_command0( uint8_t command_type, uint8_t command, uint32_t value, uint8_t direction, uint32_t *ret_value )
{
	...
	ctrl_channel_handle_command( command_type, command, value, direction, ret_value );
...
uint8_t application_api_calibration0( uint8_t type, uint8_t id, uint8_t direction, void *data, uint32_t data_size, uint32_t *ret_value )
{
    ...
    ctrl_channel_handle_api_calibration( type, id, direction, data, data_size, ret_value );
