...
#define SYSTEM_CHARDEV_FIFO_SIZE 80 * 1024
...
static int isp_fops_open( struct inode *inode, struct file *f )
{
    ...
    if ( !isp_dev_ctx.dev_inited ) {
        printk("dev is not inited, failed to open." );
        return -ERESTARTSYS;
    }
    ...
    if ( !p_ctx ) {
        printk("Fatal error, isp dev contexts is wrong, contents dump:" );
        printk("    minor_id: %d, name: %s.", isp_dev_ctx.dev_minor_id, isp_dev_ctx.dev_name );
    ...
    if ( rc ) {
        printk("Error: lock failed of dev: %s.", p_ctx->dev_name );
        goto lock_failure;
    }

    if ( p_ctx->dev_opened ) {
        printk("open(%s) failed, already opened.", p_ctx->dev_name );
        rc = -EBUSY;
    } else {
        p_ctx->dev_opened = 1;
        rc = 0;
...
static int isp_fops_release( struct inode *inode, struct file *f )
{
    ...
    if ( p_ctx != &isp_dev_ctx ) {
        printk("Inalid paramter: %p.", p_ctx );
        return -EINVAL;
    }
    ...
    if ( p_ctx->dev_opened ) {
        ...
        printk("Fatal error: wrong state of dev: %s, dev_opened: %d.", p_ctx->dev_name, p_ctx->dev_opened );
        rc = -EINVAL;
    }
...
static ssize_t isp_fops_write( struct file *file, const char __user *buf, size_t count, loff_t *ppos )
{
    ...
    if ( mutex_lock_interruptible( &p_ctx->fops_lock ) ) {
        printk("Fatal error: access lock failed." );
        return -ERESTARTSYS;
    }
...
static ssize_t isp_fops_read( struct file *file, char __user *buf, size_t count, loff_t *ppos )
{
    ...
    if ( kfifo_is_empty( &p_ctx->isp_kfifo_out ) ) {
        ...
        /* wait for the event */
        //printk("input FIFO is empty, wait for data, timeout_in_jiffies: %ld, HZ: %d.", time_out_in_jiffies, HZ );
        rc = wait_event_interruptible_timeout( p_ctx->kfifo_out_queue, !kfifo_is_empty( &p_ctx->isp_kfifo_out ), time_out_in_jiffies );

        //printk("data is coming or timeout, kfifo_out size: %u, rc: %d.", kfifo_len( &p_ctx->isp_kfifo_out ), rc );
...
static int isp_dev_context_init( struct isp_dev_context *p_ctx )
{
    ...
    if ( rc ) {
        printk("Error: register ISP device failed, ret: %d.", rc );
        return rc;
    }
    ...
    printk("isp_dev_context(%s) init OK.", p_ctx->dev_name );
...
int system_chardev_init( void )
{
    ...
    if ( rc ) {
        printk("Error: isp_dev_context_init failed for dev: %s.", p_ctx0->isp_dev.name );
        return rc;
    }
...
int system_chardev_read( char *data, int size )
{
    ...
    if ( !isp_dev_ctx.dev_inited ) {
        printk("dev is not inited, failed to read." );
        return -1;
    }
    ...
    if ( kfifo_is_empty( &isp_dev_ctx.isp_kfifo_in ) ) {
        ...
        //printk("data is coming or timeout, kfifo_in size: %u, rc: %d.", kfifo_len( &isp_dev_ctx.isp_kfifo_in ), rc );
...
int system_chardev_write( const char *data, int size )
{
    ...
    if ( !isp_dev_ctx.dev_inited ) {
        printk("dev is not inited, failed to write." );
        return -1;
    }
    ...
    //printk("wake up reader who wait on kfifo out." );
...
int system_chardev_destroy( void )
{
    if ( isp_dev_ctx.dev_inited ) {
        ...
        printk("misc_deregister dev: %s.", isp_dev_ctx.dev_name );
    } else {
        printk("dev not inited, do nothing." );
    }
