...
//		SET/GET FUNCTION
// ------------------------------------------------------------------------------ //
//set command: ret = aisp_command( ctx_id, TALGORITHMS, AF_MODE, AF_AUTO, COMMAND_SET, &RET_VALUE);
//get command: ret = aisp_command( ctx_id, TALGORITHMS, AF_MODE, AF_AUTO, COMMAND_GET, &RET_VALUE);
...
uint8_t aisp_api_calibration( uint32_t ctx_id, uint8_t type, uint8_t id, uint8_t direction, void* data, uint32_t data_size, uint32_t* ret_value);

uint8_t aisp_api_dma_buffer( uint32_t ctx_id, uint8_t type, void* data, uint32_t data_size, uint32_t* ret_value, uint32_t index);
void aisp_api_dma_buff_queue_reset(uint32_t ctx_id, uint8_t type);
uint8_t aisp_api_set_fps(uint32_t ctx_id, uint8_t type, uint32_t c_fps, uint32_t t_fps);
void aisp_api_dma_buff_get_next(uint32_t ctx_id, uint8_t type);
