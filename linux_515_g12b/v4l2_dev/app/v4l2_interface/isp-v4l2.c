...
static int isp_v4l2_fop_open( struct file *file )
{
    ...
    if (g_isp_v4l2_devs[dev->ctx_id] == NULL) {
        LOG( LOG_ERR, "video device has not register");
        rc = -1;
        return rc;
    }
...
static int isp_v4l2_init_dev( uint32_t ctx_id, struct v4l2_device *v4l2_dev )
{
    ...
    if ( ctx_id >= FIRMWARE_CONTEXT_NUMBER ) {
        LOG( LOG_ERR, "Invalid ctx number: %d, max: %d.", ctx_id, FIRMWARE_CONTEXT_NUMBER - 1 );
        return -EINVAL;
    }
    ...
    if ( rc ) {
        pr_info(" %s :isp_v4l2_ctrl_init failed \n", __func__);
        goto free_dev;
     }
    ...
    pr_info("%s: video_register_device \n", __func__);
#else
    rc = video_register_device( vfd, VFL_TYPE_GRABBER, VIDEO_NODE_NUM );
#endif
    if ( rc < 0 ) {
        pr_info("%s: video_register_device failed \n", __func__);
    ...
    for ( ctx_id = 0; ctx_id < g_firmware_context_number; ctx_id++ ) {
        ...
        if ( rc ) {
            ...
            else
            {
                ...
                LOG(LOG_CRIT, "isp_v4l2_init ctx_id: %d succeeded.", ctx_id);
                break;
            }
        }
        LOG(LOG_CRIT, "isp_v4l2_init ctx_id: %d succeeded.", ctx_id);
