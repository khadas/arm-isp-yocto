...
void sinter_strength_calculate( noise_reduction_fsm_t *p_fsm )
{
    ...
    if ( ACAMERA_FSM2CTX_PTR( p_fsm )->stab.global_manual_sinter == 0 ) //  Do not update values if manual mode
    {
        ...
        uint16_t sinter_strength1 = acamera_calc_modulation_u16( log2_gain, _GET_MOD_ENTRY16_PTR( ACAMERA_FSM2CTX_PTR( p_fsm ), sinter_strength1_idx ), _GET_ROWS( ACAMERA_FSM2CTX_PTR( p_fsm ), sinter_strength1_idx ) );
        // LOG( LOG_CRIT, "sinter_strength1 %d log2_gain %d ", (int)sinter_strength1, (int)log2_gain );
        acamera_isp_sinter_strength_1_write( p_fsm->cmn.isp_base, sinter_strength1 );
