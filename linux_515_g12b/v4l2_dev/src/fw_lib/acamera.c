...
static int32_t validate_settings(acamera_settings *settings, uint32_t ctx_num)
{
    ...
    if (settings != NULL)
    {
        ...
        for (idx = 0; idx < ctx_num; idx++)
        {
            ...
            if (ctx_set->ds1_frames && ctx_set->ds1_frames_number)
            { // check if DS frames are initialized
                if (acamera_isp_isp_global_parameter_status_ds1_pipe_read(ctx_set->isp_base))
                { // check if DS is chickened out
                    result = -1;
...
int32_t acamera_interrupt_handler()
{
    ...
    LOG(LOG_ERR, "IRQ MASK is %d", irq_mask);
