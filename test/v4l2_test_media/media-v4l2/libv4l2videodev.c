                       ...
                       struct v4l2_buffer *v4l2_buf)
{
    ...
    //media_dbg(entity->media, "queue buffer success \n");
    return ret;
                       ...
                       struct v4l2_buffer *v4l2_buf)
{
    ...
    if (ret < 0) {
        media_dbg(entity->media,
                "Error: dq buffer ret %s.\n", strerror(errno));
        return ret;
    }

    //media_dbg(entity->media, "dq buffer success \n");
    return ret;
...
void v4l2_video_crop(struct media_entity *entity, int width, int height)
{
    ...
    s_crop.r.left = 0;
    s_crop.r.top = 0;
    s_crop.r.width = width;
    s_crop.r.height = height;
    rc = ioctl(entity->fd, VIDIOC_S_SELECTION, &s_crop);
    if (rc != 0) {
