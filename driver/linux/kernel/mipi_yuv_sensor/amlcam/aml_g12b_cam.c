				       ...
				       struct v4l2_async_subdev *asd)
{
	...
	pr_info("bound called with sd 0x%px, asd 0x%px, sd->dev 0x%px, name %s", subdev, asd, subdev->dev, subdev->name);
...
static int cam_async_notifier_register(struct cam_device *cam_dev)
{
	...
	cam_dev->cam_asd[0].match_type = V4L2_ASYNC_MATCH_CUSTOM;
	cam_dev->cam_asd[0].match.custom.match = NULL;
	cam_dev->cam_asd_ptr[0] = &cam_dev->cam_asd[0];
	notifier->subdevs = (struct v4l2_async_subdev **)&cam_dev->cam_asd_ptr;
	notifier->num_subdevs = 1;
#if 0
	notifier->ops = &cam_async_notifier_ops;
#endif
	notifier->bound = cam_async_notifier_bound;
	notifier->unbind = cam_async_notifier_unbind;
	notifier->complete = cam_async_notifier_complete;
...
#if PLATFORM_G12B
static u32 read_reg(unsigned long addr)
{
	...
	if (io_addr == NULL) {
		pr_info("%s: Failed to ioremap addr\n", __func__);
		return -1;
	}
	ret = (u32)__raw_readl(io_addr);
	iounmap(io_addr);
	return ret;
}

static u32 write_reg(uint32_t val, unsigned long addr)
{
	void __iomem *io_addr;
	io_addr = ioremap_nocache(addr, 8);
	if (io_addr == NULL) {
