...
static int video_vb2_ioctl_dqbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	int ret = vb2_ioctl_dqbuf(file, priv, p);
	return ret;
}
...
static const struct v4l2_ioctl_ops aml_v4l2_ioctl_ops = {
	...
	.vidioc_dqbuf			= video_vb2_ioctl_dqbuf,
...
static int video_buff_init(struct vb2_buffer *vb)
{
	...
	dev_err(video->dev,
		"%s ++ , addr 0x%x, vaddr 0x%p\n", __func__, (unsigned int)buff->addr[AML_PLANE_A], buff->vaddr[AML_PLANE_A]);
...
static int video_start_streaming(struct vb2_queue *queue, unsigned int count)
{
	...
	rtn = media_entity_pipeline_start(entity, video->pipe);
	if (rtn) {
...
static long aml_video_ioctl2(struct file *file,
	       unsigned int cmd, unsigned long arg)
{
	long ret = video_ioctl2(file, cmd, arg);
	return ret;
}
...
static const struct v4l2_file_operations aml_v4l2_fops = {
	...
	.unlocked_ioctl = aml_video_ioctl2,
	.poll = vb2_fop_poll,
	.mmap = vb2_fop_mmap,
	.read = vb2_fop_read,
};
