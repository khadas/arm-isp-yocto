...
#define AML_CSIPHY_NAME "g12b-csi2phy-%u"
...
static int settle_int_debugfs(void) {
	debugfs_root = debugfs_create_dir("settleFile", NULL);
	...
	settle = 0x1a;
	debugfs_create_u32("settle", 0644, debugfs_root, &settle);
...
static int csiphy_of_parse_dev(struct csiphy_dev_t *csiphy_dev)
{
	...
	csiphy_dev->csi_phy = csiphy_ioremap_resource(csiphy_dev, "csi2_phy0");
	if (!csiphy_dev->csi_phy) {
		rtn = -EINVAL;
		goto error_rtn;
	}

	csiphy_dev->csi_aphy = csiphy_ioremap_resource(csiphy_dev, "aphy_reg");
	if (!csiphy_dev->csi_aphy) {
		rtn = -EINVAL;
		goto error_rtn;
	}

	csiphy_dev->csi_host = csiphy_ioremap_resource(csiphy_dev, "csi0_host");
	if (!csiphy_dev->csi_host) {
		rtn = -EINVAL;
		goto error_rtn;
	}

	csiphy_dev->mipi_clk0 = devm_clk_get(csiphy_dev->dev, "cts_mipi_csi_phy_clk0_composite");
...
static int csiphy_subdev_stream_on(struct csiphy_dev_t *csiphy_dev)
{
	...
	//link_freq = 672000000;
	pr_info("get link freq from sensor: %lld \n", link_freq);
	...
	pr_info("settle %d \n", settle);
	return csiphy_dev->ops->hw_start(csiphy_dev, 2, link_freq, settle);
}
