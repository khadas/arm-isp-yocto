...
#endif
#include "../aml_g12b_adapter.h"
#include "aml_g12b_adapter_hw.h"
...
static int adap_frontend_init(void *a_dev)
{
	...
	module_reg_write(a_dev, module, CSI2_CLK_RESET, 0x7);
	...
	module_update_bits(a_dev, module, CSI2_DDR_STRIDE_PIX, param->fe_param.fe_mem_line_stride, 4, 28);//g12b
	...
	module_reg_write(a_dev, module, CSI2_GEN_CTRL1, 0x1B);//CSI2_GEN_CTRL1
...
#endif
	module_reg_read(a_dev, ALIGN_MD, MIPI_ADAPT_IRQ_PENDING0, &val);
	module_reg_write(a_dev, ALIGN_MD, MIPI_ADAPT_IRQ_PENDING0, val);
	return val;
}
...
#endif
	/*g12b*/
	module_reg_write(a_dev, FRONTEND_MD, CSI2_CLK_RESET, 0x7);
	udelay(10);
	module_reg_write(a_dev, FRONTEND_MD, CSI2_CLK_RESET, 0x6);
	module_reg_write(a_dev, FRONTEND_MD, CSI2_CLK_RESET, 0x6);
}
...
static int adap_hw_init(void *a_dev)
{
	...
	param->fe_param.fe_int_mask = 0x5; // 0xc; 0x4 ; g12b: 0x5
...
static int adap_hw_start(void *a_dev)
{
	...
	int module = ALIGN_MD;
	module_update_bits(a_dev, module, MIPI_ADAPT_IRQ_MASK0, 1, FRONT0_WR_DONE, 1);
...
static void adap_hw_stop(void *a_dev)
{
	...
	int module = ALIGN_MD;
	module_update_bits(a_dev, module, MIPI_ADAPT_IRQ_MASK0, 0, FRONT0_WR_DONE, 1);
