...
#define AML_ADAPTER_NAME "g12b-adapter-%u"
...
static int adap_of_parse_dev(struct adapter_dev_t *adap_dev)
{
	...
	adap_dev->adap_clk = devm_clk_get(adap_dev->dev, "cts_mipi_isp_clk_composite");
...
static irqreturn_t adap_interrupt_handler(int irq, void *dev)
{
	...
	//printk("adap_interrupt_handler ++\n");

	status = adap_dev->ops->hw_interrupt_status(adap_dev);
	//mipi_adap_reg_rd(MIPI_ADAPT_IRQ_PENDING0, ALIGN_IO, &status);
				...
				struct v4l2_mbus_framefmt *format)
{
	int i = 0;
	int fmt = -1;
	const struct aml_format *am_fmt = NULL;
