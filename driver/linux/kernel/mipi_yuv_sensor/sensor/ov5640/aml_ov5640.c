...
static const struct reg_value ov5640_init_setting_30fps_1920x1080[] = {
	{0x3103, 0x11, 0xff, 0}, // system clk from pad. bit[1] = 0
	{0x3008, 0x82, 0xff, 10}, // soft reset. bit[7] = 1. sleep 5ms
	{0x3008, 0x42, 0xff, 0}, // soft power down. bit[6] = 1
	{0x3103, 0x03, 0xff, 0}, // system clk from pll bit[1] = 1.

	{0x3017, 0x00, 0xff, 0}, // pin pad for mipi
	{0x3018, 0x00, 0xff, 0}, // pin pad for mipi

	{0x3034, 0x18, 0xff, 0}, //mipi 8 bit
	{0x3035, 0x11, 0xff, 0},
	{0x3036, 0x54, 0xff, 0},
	{0x3037, 0x13, 0xff, 0}, // system control regs
	{0x3108, 0x01, 0xff, 0}, // SYSTEM ROOT DIVIDER
	...
	{0x300e, 0x45, 0xff, 0},
	{0x302e, 0x08, 0xff, 0},
	{0x4300, 0x32, 0xff, 0},// 30 yuyv  / 31 yvyu  / 32 uyvy / 33 vyuy
	{0x501f, 0x00, 0xff, 0},
	{0x5684, 0x07, 0xff, 0},//
	{0x5685, 0xa0, 0xff, 0},//
	{0x5686, 0x04, 0xff, 0},//
	...
	{0x3a11, 0x60, 0xff, 0},
	{0x3a1f, 0x14, 0xff, 0},
	{0x3008, 0x02, 0xff, 0}, // power up bit[6] = 0.
};
			    ...
			    const struct ov5640_mode_info *mode)
{
	...
	for (i = 0; i < mode->reg_data_size; ++i, ++regs) {
		...
		if (mask)
			...
			ov5640_read_reg(sensor, reg_addr, &read_val);
			pr_info("addr 0x%x, w val 0x%x, r val 0x%x \n", reg_addr, val, read_val);
		}
		if (ret)
			break;
		 ...
		 int width, int height, bool nearest)
{
	...
	mode = &ov5640_mode_data[OV5640_MODE_1080P_1920_1080];
...
static void ov5640_reset(struct ov5640_dev *sensor, int out_val)
{
	int ret = -1;
	...
	ret = gpio_direction_output(sensor->reset_gpio, out_val);
	if (0 != ret ) {
...
#if 0
/* Start streaming */
static int ov5640_start_streaming(struct ov5640_dev *ov5640)
{
	/* Start streaming */
	pr_info("sensor start stream \n");
	return ov5640_write_reg(ov5640, 0x300e, 0x45);
}

/* Stop streaming */
static int ov5640_stop_streaming(struct ov5640_dev *ov5640)
{
...
static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		pr_info("%s after set mode, bef set framefmt \n", __func__);
		...
		ret = ov5640_start_streaming(sensor);
		if (ret) {
			pr_err("Start stream failed\n");
			goto out;
		}

		pr_info("stream on success");

	} else {
		ov5640_stop_streaming(sensor);
		pr_info("stream off success\n");
	}
...
static int ov5640_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sensor->xclk = devm_clk_get(dev, "g12a_24m");
...
#if 0
	clk_freq = 24000000;//24000000
	ret = clk_set_rate(sensor->xclk, clk_freq);
	if (ret) {
	...
	ret = ov5640_set_power_on(sensor);
	if (ret) {
		pr_err("power on fail");
	}
#endif
	ret = ov5640_check_chip_id(sensor);
	if (ret) {
		pr_err("5640 not connected. still do v4l2_async_register_subdev");
		goto end_i2c_config;
	}
...
end_i2c_config:
	...
	if (ret) {
		pr_err("ov5640_init_controls fail. still do v4l2_async_register_subdev");
	}
